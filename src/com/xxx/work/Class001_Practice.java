package com.xxx.work;


import java.util.ArrayList;
import java.util.Arrays;
import java.util.HashSet;

/*
    数组:
        相同数据类型有序的集合

    作用:
        数组: 存储多个数据
        变量: 存储单个数据

    特点:
        1.定长,长度一旦确定不可改变
        2.存储相同数据类型的数据
        3.有序,有索引
        4.引用数据类型

    数组长度:
        数组名 .length

    操作数组:
        根据索引操作数组
        数组名[索引]
        索引: 从0开始,依次+1

    数组的遍历:
        普通for
        for(int i=0;i<=arr.length-1;i++){  -->i作为索引
        }
        增强for
        for(String str : arr){  --> str数组中的每一个数据
        }
 */
public class Class001_Practice {
    public static void main(String[] args) {
        /*int[] arr = {4,2,8,0,1,5,0,7,10};
        //arr = null;
        System.out.println(findData(arr,0));;

        System.out.println(Arrays.toString(arr));
        System.out.println(Arrays.toString(chackZero(arr)));;

        System.out.println(Arrays.toString(change(new char[]{'A','B','C','~'})));*/

        test();
    }

    /*1.数组元素查找(查找指定元素第一次在数组中出现的索引)
         若存在:返回这个元素的下标
         若不存在:返回-1
     */
     public static int  findData(int[] arr,int value){
         if(arr!=null){
             for(int i=0;i<arr.length;i++){
                 if(value==arr[i]){
                     return i;
                 }
             }
         }
         return -1;
     }

     /*
        2.
        数组元素去0
        原数组: 4,2,8,0,1,5,0,7,10
        去0之后: 4,2,8,1,5,7,10
     */
    public static int[] chackZero(int[] arr){
        //1)找到不为0数据的个数
        int count = 0; //计数器
        for(int  i:arr){
            if(i!=0){
                count++;
            }
        }
        //2)判断原数组中是否粗在0,不存在0直接返回原数组,存在0...
        if(count==arr.length){
            return arr;
        }

        //3)存在0,创建新数组,长度为不为0数据的个数
        int[] newArr = new int[count];

        //4)不为0数据的拷贝
        //i作为原数组的索引
        int j = 0; //新数组索引
        for(int i=0;i<=arr.length-1;i++){
            if(arr[i]!=0){
                newArr[j++] = arr[i];
            }
        }
        //5)返回新数组
        return newArr;
    }

    /*
        3.将一个存放了大写字母的数组转换成存放小写字母的数组返回
     */
    public static char[] change(char[] arr){
        for(int i=0;i<arr.length;i++){
            if(arr[i]>='A' && arr[i]<='Z'){
                arr[i]+=32;
            }
        }
        return arr;
    }

    /*
        4.
            练习: 定义员工类型,存储多个员工类型的数据
            1.展示输出所有员工信息
            2.找到所有员工的姓名存储起来
            3.找到所有薪资>10000的员工->裁员
            4.找到出现的员工年龄统计个数
     */
    public static void test(){
        //定义存储员工数据的数据
        Employee[] emps = {
                new Employee("老裴",35,20000),
                new Employee("老向",34,18000),
                new Employee("老薛",33,7000),
                new Employee("老李",34,9000),
        };
        //1.展示输出所有员工信息
        for(Employee emp:emps){
            System.out.println(emp);
        }
        //2.找到所有员工的姓名存储起来
        String[] names = new String[emps.length];
        for(int i=0;i<=emps.length-1;i++){
            names[i] = emps[i].getName();
        }
        System.out.println(Arrays.toString(names));

        // 3.找到所有薪资>10000的员工->裁员
        //集合: ArrayList : 内部底层是数组,通过封装后能够实现通过需求对集合中存储的数据动态进行改动
        ArrayList arrayList = new ArrayList();  //能够存储多个数据
        for(Employee e:emps){
            if(e.getSalary()<=10000){
                arrayList.add(e);
            }
        }
        System.out.println(arrayList);

        //集合: HashSet : 默认去重,无序的
        HashSet set = new HashSet();
        for(Employee e:emps){
            set.add(e.getAge());
        }

        //获取集合中数据的个数
        System.out.println(set.size());
    }



}
